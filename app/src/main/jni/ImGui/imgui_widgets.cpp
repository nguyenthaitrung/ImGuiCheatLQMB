...
bool ImGui::ReadOnlyCheckbox(const char* label)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImU32 text_color = IM_COL32(255, 0, 0, 255);  // Màu đỏ: RGB(255, 0, 0)
    ImVec2 text_size = CalcTextSize("X");
    ImVec2 text_pos = check_bb.Min + (check_bb.GetSize() - text_size) * 0.5f;
    window->DrawList->AddText(text_pos, text_color, "X");
